...
/// <summary>
/// Strict validation for the Scanner item-database relationship graph. Relationship
/// data is user-facing canonical content, so dangling references are rejected before
/// activation instead of being guessed or silently displayed.
/// </summary>
public sealed class ItemRelationshipIntegrityValidator
{
    public ContentValidationResult Validate(GameContentCatalog content)
    {
        ...
        foreach (var purchase in relationships.TraderPurchases)
        {
            ...
            var key = $"{purchase.ItemId}\u001f{purchase.TraderId}\u001f{purchase.RequiredLevel}\u001f{purchase.TaskUnlockQuestId}";
        ...
        ICollection<ContentValidationIssue> issues)
    {
        if (string.IsNullOrWhiteSpace(itemId) || !ids.Contains(itemId))
            Fatal(issues, code, $"{owner} references missing item '{itemId}'.");
    }

    private static void RequireTrader(
        IReadOnlySet<string> ids,
        string traderId,
        string code,
        string owner,
        ICollection<ContentValidationIssue> issues)
    {
        if (string.IsNullOrWhiteSpace(traderId) || !ids.Contains(traderId))
            Fatal(issues, code, $"{owner} references missing trader '{traderId}'.");
    }

    private static void RequireQuest(
        IReadOnlySet<string> ids,
        string? questId,
        string code,
        string owner,
        ICollection<ContentValidationIssue> issues)
    {
        if (questId is not null && !ids.Contains(questId))
            Fatal(issues, code, $"{owner} references missing unlock quest '{questId}'.");
    }
    ...
    {
        if (level < 0)
            Fatal(issues, code, $"{owner} has negative required level '{level}'.");
    }
    ...
    {
        if (value <= 0)
            Fatal(issues, code, $"{owner} has non-positive count '{value}'.");
    }
