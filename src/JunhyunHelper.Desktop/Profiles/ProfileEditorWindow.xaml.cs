...
public sealed class TraderEditorRow : INotifyPropertyChanged
{
    ...
    public bool IsStandingKnown
    {
        ...
        set
        {
            if (value == IsStandingKnown) return;
...
public partial class ProfileEditorWindow : Window
{
    ...
    private const int LastCoreTraderRank = 8;
        ...
        GameProfileSnapshot? existingProfile = null)
    {
        ...
        if (existingProfile is not null && existingProfile.GameMode != gameMode)
            throw new ArgumentException("Existing profile game mode does not match the editor game mode.", nameof(existingProfile));

        InitializeComponent();

        _editingExistingProfile = existingProfile is not null;
        _level = existingProfile?.Level ?? 1;
        _prestigeLevel = existingProfile?.PrestigeLevel ?? 0;
        ...
        FactionComboBox.SelectedItem = factions.First(choice => choice.Value == (existingProfile?.Faction ?? PmcFaction.Usec));

        var editions = new[] { new EditionChoice(null, "미입력") }
            .Concat(content.Editions.OrderBy(edition => edition.Title, StringComparer.CurrentCulture)
        ...
        _traderRows = content.Traders.Select(trader =>
            {
                TraderProgress progress = default;
                var hasProgress = existingProfile is not null && existingProfile.Traders.TryGetValue(trader.Id, out progress);
        ...
        var coreRows = _traderRows.Where(row => !row.IsFence && row.DisplayRank <= LastCoreTraderRank)
            .OrderBy(row => row.DisplayRank).ThenBy(row => row.Name, StringComparer.CurrentCulture).ToArray();
        TraderItems.ItemsSource = coreRows;

        var specialRows = _traderRows.Where(row => !row.IsFence && row.DisplayRank > LastCoreTraderRank)
            .OrderBy(row => row.DisplayRank).ThenBy(row => row.Name, StringComparer.CurrentCulture).ToArray();
        SpecialTraderItems.ItemsSource = specialRows;
        SpecialTraderExpander.Visibility = specialRows.Length > 0 ? Visibility.Visible : Visibility.Collapsed;
        SpecialTraderExpander.IsExpanded = false;
    ...
    private void PrestigeMinusButton_Click(object sender, RoutedEventArgs e) { _prestigeLevel = Math.Max(0, _prestigeLevel - 1); UpdateTopValues(); }
    private void PrestigePlusButton_Click(object sender, RoutedEventArgs e) { _prestigeLevel++; UpdateTopValues(); }

    private static TraderEditorRow? RowFrom(object sender) => (sender as FrameworkElement)?.DataContext as TraderEditorRow;

    private void TraderLoyaltyMinusButton_Click(object sender, RoutedEventArgs e)
    {
        if (RowFrom(sender) is not { IsFence: false } row) return;
    ...
    private void TraderStandingMinusButton_Click(object sender, RoutedEventArgs e) => AdjustStanding(sender, -0.1m);
    private void TraderStandingPlusButton_Click(object sender, RoutedEventArgs e) => AdjustStanding(sender, 0.1m);

    private static void AdjustStanding(object sender, decimal delta)
    {
        if (RowFrom(sender) is not { } row || row.Standing is null) return;
    ...
    private void SaveButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        foreach (var row in _traderRows)
        {
            ...
            if (loyalty is null && standing is null) continue;
    ...
    private void UpdateTopValues()
    {
        ...
        PrestigeValueText.Text = _prestigeLevel.ToString(CultureInfo.CurrentCulture);
    }
    ...
    private sealed record FactionChoice(PmcFaction Value, string Label) { public override string ToString() => Label; }
    private sealed record EditionChoice(string? Id, string Label) { public override string ToString() => Label; }
}
