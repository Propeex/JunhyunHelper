...
- Mini Scanner는 별도로 Tarkov 상단 inventory/stash context OCR을 반복 수행한다.
...
## 4. Inventory/stash context OCR 역할 변경

`ScannerInventoryContextDetector`의 보조 context OCR 자체는 제거하지 않는다.

새 계약:

- hidden Mini Scanner가 처음 열릴 때는 기존처럼 foreground Tarkov inventory/stash context를 확인한다.
...
스캔 주기 `350 ms`, semantic retry `1200 ms`도 이번 안정화 PATCH에서는 측정 근거 없이 변경하지 않는다.
...
`docs/VERSIONING.md`의 PATCH 규칙에 따라 목표 버전은 **v1.7.2**다.

## 9. 완료 조건

- deterministic retention tests 통과
- 전체 automated tests 0 failed / 0 skipped
- Windows x64 publish 성공
- Product UI / Scanner / Mini Scanner rendered smoke 성공
- Map / Factory / MiniMap smoke 성공
- Scanner recognition threshold/candidate cap diff 없음
- PR CI 성공 후 main 병합
- main CI 성공
- v1.7.2 공개 release 및 stable ZIP/manifest 검증
- 공식 상태 문서와 release notes 갱신
