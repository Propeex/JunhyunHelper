...
다만 current EFT 1.1 live dataset에서 `globalVariable` 162 Quest가 정확히 27개의 trader-local staged task-pool 변수로 묶이고, LL2~LL4에 대해 같은 trader의 direct loyalty-level seed batch가 현재 source에서 교차검증되는 구조는 제품의 current Quest 표시에서 **버전 고정 compatibility**로 복원할 수 있다.
...
2. exact 값이 없고 audited current-version shape가 완전히 일치하면 LL2~LL4 task-pool만 runtime reconstruction을 허용한다.
3. 위 조건이 성립하지 않으면 기존 `Indeterminate / 확인 필요`로 fail closed한다.
...
LL1 task-pool 48 Quest에는 같은 방식으로 current counter의 시작을 증명하는 public direct-LL seed/write rule이 없다.

따라서:

- LL1 missing value를 0으로 가정하지 않는다.
- 완료 Quest 수만으로 현재 counter 전체를 임의 재구성하지 않는다.
...
`FutureNeededItemsPlanner`는 이 runtime reconstruction을 사용하지 않고 기존 conservative future reachability를 유지한다. missing profile-variable future fact는 계속 `IndeterminatePotential`로 보호한다.

따라서 이 결정은 필요한 미래 Item을 잘못 `정리 가능`으로 만드는 방향으로 사용하지 않는다.

## 이유

v0.1.8에서는 canonical read-side condition을 정확히 보존했지만 exact profile value가 없다는 이유만으로 162 usage 전체가 structural unknown이 되어 사용자에게 150개 이상의 `확인 필요`를 노출했다.
...
current-version compatibility 이후 exact LL1 value가 없다는 가정의 raw unresolved ceiling은 61(48 + 13)이다.

이는 source가 제공하지 않는 generic server write rule을 발명하지 않으면서도, current source가 실제로 반복해서 제공하는 검증 가능한 단계형 구조를 제품에 이용하는 절충이다.

## 장기 방향

향후 scanner/importer가 EFT profile payload에서 exact `Variables` dictionary를 안전하게 가져올 수 있으면 exact 값 사용이 우선하며 이 compatibility 의존도는 줄어든다.
