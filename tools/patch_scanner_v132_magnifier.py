...
path = Path('src/JunhyunHelper.Desktop/Scanner/ScannerTitleAnchorRefiner.cs')
text = path.read_text(encoding='utf-8')

old = '''        var fieldHeight = titleField.Height > 0 ? titleField.Height : fallbackTitle.Height;
        var left = Math.Clamp(
...
new = '''        var fieldHeight = titleField.Height > 0 ? titleField.Height : fallbackTitle.Height;
        var left = Math.Clamp(
            panel.X - Math.Max(8, (int)Math.Round(panel.Width * 0.04)),
            0,
            width - 1);

        // Real v1.3.1 captures show a clean ~21 px magnifier component can be lost when
        // refined title-strip vertical association drifts. Search the union of structural
        // panel-top, fallback-title and title-field bands; morphology/position decides it.
...
if old not in text:
    raise SystemExit('magnifier search-band source block not found')
text = text.replace(old, new, 1)

old = '''            var textFollowers = components
                .Skip(index + 1)
                .Where(value =>
...
new = '''            var textFollowers = components
                .Where(value =>
                    value.X > component.X + component.Width &&
                    value.X - (component.X + component.Width) <= Math.Max(20, (int)Math.Round(fieldHeight * 1.4)) &&
                    IsTextLike(value, fieldHeight))
            ...
            var dominance = hasFollowers
                ...
                : 0.0;'''
if old not in text:
    raise SystemExit('magnifier follower source block not found')
text = text.replace(old, new, 1)

old = '''            var leftOrder = components.Count <= 1
                ? 1.0
...
new = '''            var expectedLeft = panel.X;
            ...
            // Following title text is corroboration, not a prerequisite. Real anti-
            // aliased glyphs can fragment differently while the magnifier remains a
            // strong ring/hollow-center/handle shape. Without followers, require both
            // strong morphology and the expected left-header position.
            if (hasFollowers)
            {
                if (dominance < 1.08 && morphology < 0.68)
                    continue;
            ...
            var score =
                ...
                followerScore * 0.08;'''
if old not in text:
    raise SystemExit('magnifier scoring source block not found')
text = text.replace(old, new, 1)
path.write_text(text, encoding='utf-8')

smoke = Path('src/JunhyunHelper.Desktop/Scanner/ScannerPage.MiniScannerSmoke.cs')
s = smoke.read_text(encoding='utf-8')
needle = '''        if (refinement.Magnifier.Width < 20 ||
...
'''
addition = needle + '''
        // Live-like regression: leave the magnifier intact but make the first nearby
        // text component too sparse/far to satisfy the old mandatory-follower relation.
        // Strong magnifier morphology + left-header position must still exclude it from OCR.
...
'''
if needle not in s:
    raise SystemExit('scanner smoke insertion marker not found')
smoke.write_text(s.replace(needle, addition, 1), encoding='utf-8')
