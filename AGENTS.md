...
**개발자 재량은 확인된 제품 동작을 보존하는 내부 구현에 한정합니다.** 성능 최적화, 구조 개선, 테스트 강화, 캐시/동시성/알고리즘 교체처럼 사용자 의미를 바꾸지 않는 작업은 자율적으로 할 수 있지만, 구현 중 빈칸을 메우기 위해 새로운 제품 의미를 발명하면 안 됩니다.

특히 다음은 내부 구현 세부사항으로 취급하지 않고 **사용자 확인이 필요한 제품 결정**으로 봅니다.

- 프로그램이 어떤 사실을 참/거짓으로 간주하는 새로운 판정 규칙
- 기존에 없던 화면 관측/OCR/아이콘/색상/외부 데이터 등을 새로운 권위로 채택하는 것
- 새로운 자동 추론이나 분류 규칙
- 새로운 사용자 입력·확인·팝업·설정·실패 흐름
- `판단 보류`, `버리기`, 자동 반영 등 사용자에게 의미 있는 결과 조건의 변경
- 기능 사이에 새로운 자동 연동이나 상태 전파를 추가하는 것

이런 결정이 공식 제품/결정 문서에 이미 확정되어 있지 않다면, 구현 편의를 이유로 먼저 만들지 않습니다. **제품 의미가 달라질 수 있는 선택지가 생기면 사용자에게 확인한 뒤 구현합니다.**
