...
개발자는 성능, 안정성, 유지보수성, 탐색 예산, 캐시, 자료구조, 병렬화 같은 **내부 구현 방법**은 제품 의미를 보존하는 범위에서 독립적으로 개선할 수 있습니다. 그러나 구현 편의나 최적화를 이유로 다음을 새로 만들거나 바꾸면 안 됩니다.

- 제품의 판단 기준을 추가·삭제·재정렬하는 것
- 프로그램이 사실을 관찰·추론하는 새로운 권한이나 데이터 출처를 만드는 것
- 새로운 자동 추론, 자동 확인, 사용자 입력·확인 절차를 만드는 것
- 기능 간 자동 연동이나 숨은 cross-feature 동작을 만드는 것
- 사용자에게 보이는 성공·실패·보류·파괴적 조치의 의미를 바꾸는 것

이 중 하나라도 필요해 보이면 **기술 구현 문제가 아니라 제품 의미 변경 후보**로 취급하고 사용자 의도를 먼저 확정합니다. 확인되지 않은 사실을 `0`, `없음`, `1x1` 같은 편의 기본값으로 꾸며 파괴적 결정을 정당화하지 않으며, 확정된 제품 계약이 별도로 허용하지 않는 한 사실 증명이 부족한 파괴적 자동 판단은 fail-closed로 처리합니다.
