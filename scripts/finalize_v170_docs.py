...
def section(path: str, start: str, end: str, replacement: str) -> None:
    ...
    if count != 1:
        raise RuntimeError(f"{path}: section {start!r} -> {end!r} matched {count} times")
    write(path, updated)


def tail(path: str, start: str, replacement: str) -> None:
    text = read(path)
    pattern = re.escape(start) + r".*\Z"
    updated, count = re.subn(pattern, replacement.rstrip() + "\n", text, count=1, flags=re.S)
    if count != 1:
        raise RuntimeError(f"{path}: tail {start!r} matched {count} times")
    write(path, updated)


def literal(path: str, old: str, new: str, required: bool = True) -> None:
    text = read(path)
    count = text.count(old)
    if required and count != 1:
        raise RuntimeError(f"{path}: literal {old!r} matched {count} times")
    if not required and count > 1:
        raise RuntimeError(f"{path}: optional literal {old!r} matched {count} times")
    if count == 1:
...
""")
section("README.md", "## 릴리즈 상태\n", "## 주요 기능", release_block)

readme_changes = block("""
## v1.7.0 주요 변경
...
""")
section("README.md", "## v1.6.0 주요 변경\n", "## Scanner", readme_changes)
literal("README.md", "## Scanner 사용 흐름 — v1.6.0", "## Scanner 사용 흐름 — current", required=False)

literal("docs/STATE.md", "기준일: 2026-08-24", "기준일: 2026-08-25")
literal("docs/STATE.md", "상태: **v1.6.0 PUBLIC RELEASE / VERIFIED — LIVE GROUND TRUTH MAINTENANCE**", "상태: **v1.7.0 PUBLIC RELEASE / VERIFIED — LIVE GROUND TRUTH MAINTENANCE**")
state_release = block("""
...
- `docs/.release-v1.7.0-public-proof.json`
""")
section("docs/STATE.md", "## 2. 공개 stable과 현재 source\n", "## 3. 아키텍처", state_release)
for old, new in (
    ("v1.6.0 일반 Scanner 화면에는 catalog force-refresh action을 노출하지 않는다.", "현재 일반 Scanner 화면에는 catalog force-refresh action을 노출하지 않는다."),
    ("One-shot 기능은 v1.6.0에서도 유지한다.", "One-shot 기능은 현재도 유지한다."),
    ("v1.6.0 Scanner item search는 같은 current full-item catalog와 local presentation data를 사용한다.", "현재 Scanner item search는 같은 current full-item catalog와 local presentation data를 사용한다."),
    ("## 11. Scanner 일반 UI — v1.6.0", "## 11. Scanner 일반 UI — current"),
):
    literal("docs/STATE.md", old, new, required=False)

literal("docs/CURRENT_STATE.md", "기준일: 2026-08-24", "기준일: 2026-08-25")
literal("docs/CURRENT_STATE.md", "상태: **`v1.6.0 PUBLIC RELEASE / VERIFIED — LIVE GROUND TRUTH MAINTENANCE`**", "상태: **`v1.7.0 PUBLIC RELEASE / VERIFIED — LIVE GROUND TRUTH MAINTENANCE`**")
current_release = block("""
...
""")
section("docs/CURRENT_STATE.md", "## 공개 stable과 현재 source\n", "## 기능 상태", current_release)
literal("docs/CURRENT_STATE.md", "| Program Update | 구현 완료 / v1.6 stable ZIP contract 반영 |", "| Program Update | 구현 완료 / stable ZIP + immutable published-release contract |", required=False)
literal("docs/CURRENT_STATE.md", "| Scanner + Mini Scanner | **v1.6.0 공개 검증 완료 / live Ground Truth maintenance** |", "| Scanner + Mini Scanner | **v1.7.0 공개 검증 완료 / live Ground Truth maintenance** |", required=False)
current_changes = block("""
## v1.7.0 핵심 변경
...
""")
section("docs/CURRENT_STATE.md", "## v1.6.0 핵심 변경\n", "## Scanner 안전 기준선", current_changes)

literal("docs/CURRENT_SCANNER_WORK.md", "기준일: 2026-08-24", "기준일: 2026-08-25")
literal("docs/CURRENT_SCANNER_WORK.md", "상태: **v1.6.0 PUBLIC RELEASE / VERIFIED — LIVE GROUND TRUTH MAINTENANCE**", "상태: **v1.7.0 PUBLIC RELEASE / VERIFIED — LIVE GROUND TRUTH MAINTENANCE**")
literal("docs/CURRENT_SCANNER_WORK.md", "Scanner v1.6.0 사용자 작업 흐름 정리와 공개 릴리즈 검증이 완료되었다.", "Scanner v1.7.0 Product Completion hardening과 공개 릴리즈 검증이 완료되었다.", required=False)
scanner_goal = block("""
## v1.7.0 완료 내용
...
""")
section("docs/CURRENT_SCANNER_WORK.md", "## v1.6.0 목표\n", "## 현재 production recognition pipeline", scanner_goal)
literal("docs/CURRENT_SCANNER_WORK.md", "v1.6.0 UI 작업으로 다음 값을 변경하지 않았다.", "v1.7.0 hardening에서도 다음 값을 변경하지 않았다.", required=False)
literal("docs/CURRENT_SCANNER_WORK.md", "## v1.6.0 Scanner 일반 UI", "## Scanner 일반 UI — current", required=False)

literal("docs/PRODUCT.md", "기준일: 2026-08-24", "기준일: 2026-08-25")
literal("docs/PRODUCT.md", "상태: **v1.6.0 RELEASE CANDIDATE / FINAL RELEASE GATE**", "상태: **v1.7.0 PUBLIC RELEASE / VERIFIED**")
literal("docs/PRODUCT.md", "현재 public stable은 v1.5.0이고, current source는 v1.6.0 release candidate다. v1.6.0은 공개 검증 전 public stable로 간주하지 않는다.", "현재 public stable/latest는 v1.7.0이다. 제품 source의 권위는 tag `v1.7.0`의 exact SHA `56e12342e3490fd0defa5f327a03d20d4f32b3a6`이며, main의 후속 문서/housekeeping commit은 release source가 아니다.")

literal("docs/ARCHITECTURE.md", "기준일: 2026-08-24", "기준일: 2026-08-25")
literal("docs/ARCHITECTURE.md", "상태: **v1.6.0 RELEASE CANDIDATE**", "상태: **v1.7.0 PUBLIC RELEASE / VERIFIED**")

literal("docs/DEVELOPER_REFERENCE.md", "상태: **ACTIVE / v1.6.0 RELEASE CANDIDATE**", "상태: **ACTIVE / v1.7.0 PUBLIC RELEASE VERIFIED**")
literal("docs/DEVELOPER_REFERENCE.md", "기준일: 2026-08-24", "기준일: 2026-08-25")
dev_baseline = block("""
현재 public stable/latest는 **v1.7.0**이다.
...
- `docs/.release-v1.7.0-public-proof.json`
""")
section("docs/DEVELOPER_REFERENCE.md", "현재 public stable은 v1.5.0이다.\n", "`vendor/Tarkov-Helper`", dev_baseline)

literal("docs/SCANNER.md", "기준일: 2026-08-24", "기준일: 2026-08-25")
literal("docs/SCANNER.md", "상태: **v1.6.0 RELEASE CANDIDATE**", "상태: **v1.7.0 PUBLIC RELEASE / VERIFIED — LIVE GROUND TRUTH MAINTENANCE**")
scanner_scope = block("""
...
""")
section("docs/SCANNER.md", "## 2. v1.6.0 release scope\n", "## 3. 핵심 안전 불변식", scanner_scope)

literal("docs/SCANNER_GROUND_TRUTH.md", "기준일: 2026-08-24", "기준일: 2026-08-25")
literal("docs/SCANNER_GROUND_TRUTH.md", "상태: **v1.6.0 RELEASE CANDIDATE / ACTIVE**", "상태: **v1.7.0 PUBLIC RELEASE VERIFIED / ACTIVE**")
literal("docs/SCANNER_GROUND_TRUTH.md", "이 문서는 준현 헬퍼 Scanner의 실사용 성공·실패를 재현 가능한 개발 데이터로 전환하는 공식 계약이다. `docs/SCANNER.md`의 production recognition pipeline을 폐기하지 않고 그 위에 진단, 사용자 교정, Ground Truth, replay regression 체계를 둔다.", "이 문서는 준현 헬퍼 Scanner의 실사용 성공·실패를 재현 가능한 개발 데이터로 전환하는 공식 계약이다. `docs/SCANNER.md`의 production recognition pipeline을 폐기하지 않고 그 위에 진단, 사용자 교정, Ground Truth, replay regression 체계를 둔다. v1.7.0부터 recognition log의 교정 action도 동일 Case ID의 저장된 diagnostic evidence 또는 정확히 같은 current frame이 있을 때만 이 계약으로 진입한다.")

literal("docs/SCANNER_TEST_PLAN.md", "# Scanner v1.6.0 Test Plan", "# Scanner Current Test Plan")
literal("docs/SCANNER_TEST_PLAN.md", "기준일: 2026-08-24", "기준일: 2026-08-25")
literal("docs/SCANNER_TEST_PLAN.md", "상태: **v1.6.0 RELEASE CANDIDATE / FINAL RELEASE GATE**", "상태: **v1.7.0 PUBLIC RELEASE VERIFIED / LIVE GROUND TRUTH MAINTENANCE**")
test_current = block("""
...
""")
section("docs/SCANNER_TEST_PLAN.md", "## 1. Current release candidate\n", "## 2. Release-blocking gate", test_current)
literal("docs/SCANNER_TEST_PLAN.md", "Scanner normal v1.6 surface smoke", "Scanner current normal surface smoke", required=False)
literal("docs/SCANNER_TEST_PLAN.md", "exact source tag `v1.6.0`", "exact source tag `v1.7.0`", required=False)

literal("docs/DECISION_V1.7.0_PRODUCT_COMPLETION_2026-08-24.md", "상태: `IMPLEMENTED / RELEASE CANDIDATE VALIDATION`", "상태: `IMPLEMENTED / PUBLIC RELEASE VERIFIED`")
...
""")
tail("docs/DECISION_V1.7.0_PRODUCT_COMPLETION_2026-08-24.md", "## 8. Release Candidate 검증 현황\n", decision_final)

literal("docs/RELEASE_1.7.0.md", "상태: `RELEASE CANDIDATE VALIDATION`", "상태: `PUBLIC RELEASE VERIFIED`")
release_final = block("""
## 자동/공개 검증 — FINAL
...
""")
tail("docs/RELEASE_1.7.0.md", "## 자동 검증 — Release Candidate\n", release_final)

status_doc = textwrap.dedent("""
...
""").strip() + "\n"
Path("docs/STATUS_V1.7.0_PRODUCT_COMPLETION_2026-08-25.md").write_text(status_doc, encoding="utf-8", newline="\n")

print("v1.7.0 documentation finalization prepared successfully")
